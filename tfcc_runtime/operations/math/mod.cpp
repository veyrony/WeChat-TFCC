...
template <class T>
static const char* _wrapper_mod_v1(
...
template <class T>
static const char* _wrapper_mod_v2(
...
template <class T>
static const char* _wrapper_mod_v3(
...
template <class T>
static const char* _wrapper_mod_v4(const T* a, const T* b, T* result) noexcept {
  *result = *a - *a / *b * *b;
  return nullptr;
}

template <tfcc::runtime::common::DataType DTYPE>
std::string Mod<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::math::Mod operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> Mod<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::math::Mod::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> Mod<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 2 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::math::Mod>()) {
    return nullptr;
  }
...
std::vector<std::unique_ptr<Operation>> get_mod_operations() {
#define DEFINE_FUNC(dtype) operations.emplace_back(std::unique_ptr<Operation>(new Mod<dtype>()));
